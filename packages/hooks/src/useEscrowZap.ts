...
export const useEscrowZap = ({
  ...
  const tokenAddress =
    _.get(networkConfig[chainId], `TOKENS.${token}.address`) ?? '0x';

  const resolver = daoSplit
    ? (_.first(_.keys(_.get(networkConfig[chainId], 'RESOLVERS'))) as Hex)
    : (networkConfig[chainId].DAO_ADDRESS ?? '0x');
  ...
  const {
    ...
    chainId,
    address: networkConfig[chainId].ZAP_ADDRESS ?? '0x',
    abi: ESCROW_ZAP_ABI,
...
interface UseEscrowZapProps {
  ...
  detailsData: ProjectDetails | null;
  enabled?: boolean;
